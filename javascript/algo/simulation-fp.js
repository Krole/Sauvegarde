const cardMax = 14;
let pot = 0;
let oop = { // oop et ip sont 2 joueurs
    money: 1000
};
...
function results(){
        if (oop.card>ip.card || ip.move ==='FOLD'){ // les || pour le ou booleens
             oop.money += pot //ecriture pour faire oop.money = oop.money + pot
...
function simulation(){
    ...
    let ipResult = []; //let = var
    // let altResult = {};

    for (let limit = 2 ; limit <= 14; limit ++){
        ip.money = 1000;
        oop.money = 1000;
        for (let i = 0 ; i < 10000 ; i++){ // up to 1M
